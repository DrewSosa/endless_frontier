...
var init = function(){
	...
	return {
		...
		w:1436,
		h:782,
		// don't really understand data_raw
		data_raw:{},
		...
		getData:function(callback){
			var self = vis,
				sets = ['policy_topics','policy_theories','policy_theories_links'],
				setv = 'policy_theories_votes'
				;
		...
		processData:function(){
			...
			self.data_raw.policy_topics.forEach(function(d){
				d.size_scaled = scale_size(d.size);
				self.policy_topics[d.id] = d;

				d.description_HTML = d.description;

				//make links HTML-ready
				if(d.links){
					// ATTN: ensure that the json descriptions have matching parentheses!
			...
			self.data_raw.policy_theories.forEach(function(d){
				self.policy_theories[d.id] = d;
				self.policy_theories[d.id].topic = [];
				self.policy_theories[d.id]['t_parents']  = [];
				self.policy_theories[d.id]['t_children'] = [];
				self.policy_theories[d.id]['t_siblings'] = [];
			...
			self.data_raw.policy_theories_votes.forEach(function(d){
				if(self.policy_theories[d.title]){
					self.policy_theories[d.title].WP = d;
				}
			});
			...
			self.data_raw.policy_topics.forEach(function(d){
				...
				if(d.policy_theories){
					d.policy_theories.forEach(function(_d){
						var t_node;
						// have legit no idea what this means??
						t_node = self.t_nodes.indexOf(self.policy_theories[_d]) <0 ? self.policy_theories[_d] : $.extend(true, {}, self.policy_theories[_d]);
						t_node.policy_topic = d.id;
					...
					d.policy_theories.sort(function(a,b){
						var _a = self.policy_theories[a].WP ? parseInt(self.policy_theories[a].WP.count) : 0,
							_b = self.policy_theories[b].WP ? parseInt(self.policy_theories[b].WP.count) : 0;
						if (_a > _b) {
							return -1;
						}
						if (_a < _b) {
			...
			self.data_raw.policy_topics.forEach(function(d,i){
				if(d.children){
					d.children.forEach(function(_d){
						self.links.push({
							'source':i,
							'target':d3.keys(self.policy_topics).indexOf(_d.id),
							'value':_d.overlap
						});
					});
				}
			...
			self.data_raw.policy_theories_links.forEach(function(d,i){
		...
		draw:function(){
			...
			self.t_hBoxVote
				.on('click',function(){
					theoryVote(self.policy_theories[self.theoryFocus],this);
				});
			self.t_hBoxTweet
				.on('click',function(){
					theoryTweet(self.policy_theories[self.theoryFocus]);
				});

			var alpha = [],
				top_arr = d3.keys(self.policy_topics).sort();
			top_arr.forEach(function(d){
				var obj = {
					'id':d,
					'name':self.policy_topics[d].name,
					'topic':true
				};
				alpha.push(obj);
				if(self.policy_topics[d].policy_theories){
					var t_arr = self.policy_topics[d].policy_theories.slice();
					...
					t_arr.forEach(function(_d){
						var t_obj = {
							'id':_d,
							'name':self.policy_theories[_d].name,
							'parent':d,
							'topic':false
						}
						alpha.push(t_obj);
			...
			function colClick(d){
				if(d.policy_topic){
					...
					var _t = self.t_nodes.filter(function(_d){
						return _d.policy_topic === d.parent && _d.id === d.id;
					})[0];
				...
				t_nodes
					.classed('spotlight',function(d){
						return self.topicFocus && d.policy_topic === self.topicFocus;
					})
					.attr('r',function(d){
						if(self.topicFocus && d.policy_topic === self.topicFocus){
							d.radius = c_rad;
						} else{
							d.radius = t_rad;
						}
			...
			col_01
				.classed('topic',function(d){
					return d.policy_topic;
				})
				.each(function(d,i){
					d.first = i === 0;
				})
				.style('font-size',function(d){
					return d.policy_topic ? topicFont +'px' : theoryFont +'px';
				})
				.style('margin-left',function(d){
					return d.policy_topic ? '0px' : topicFont +'px';
				})
				.style('padding-top',function(d,i){
					return d.first ? '0px' : d.policy_topic ? topicPad +'px' : theoryPad +'px';
				})
			...
			col_03.enter().append('div')
				.classed('theoryLink',true);
			col_03
				.classed('policy_topic',function(d){
					return d.policy_topic;
				})
				.each(function(d,i){
					d.first = i === 0;
			...
			self.svg
				...
				.on('mousemove',function(){
					...
					if(!self.theoryFocus){
						...
						console.log("I don't think we get here at alll");
						...
						t_nodes
							...
							.classed('semihighlight',function(d){
								return d.policy_topic === focus.id && focus.distance <self.focusDistance && (self.topicFocus && d.policy_topic !== self.topicFocus);
							})
							.classed('highlight',function(d){
								return d.policy_topic === focus.id && (focus.distance <self.focusDistance || self.topicFocus && d.policy_topic === self.topicFocus);
							})
							.attr('r',function(d){
								if(!self.topicFocus && d.policy_topic === focus.id && focus.distance <self.focusDistance){
									d.radius = f_rad;
								} else if(self.topicFocus && d.policy_topic === self.topicFocus){
									d.radius = c_rad;
								} else{
									d.radius = t_rad;
								}
			...
			//TOPICS
			policy_topics = self.svgG.selectAll('g.policy_topics')
				.data([force]);
			policy_topics.enter().append('g')
				.classed('policy_topics',true);
			policy_topics.exit().remove();
			links = policy_topics.selectAll('path.link')
			...
			links.exit().remove();
			hulls = policy_topics.selectAll('path.hull')
			...
			//THEORIES
			policy_theories = self.svgG.selectAll('g.policy_theories')
				.data([t_force]);
			policy_theories.enter().append('g')
				.classed('policy_theories',true);
			policy_theories.exit().remove();
			t_links = policy_theories.selectAll('path.t_link')
				.data(function(d){ return d.links(); });
			t_links.enter().append('path')
				.classed('t_link',true);
			t_links.exit().remove();
			t_nodes = policy_theories.selectAll('circle.t_node')
				.data(function(d){ return d.nodes(); })
				;
			...
			t_nodes.exit().remove();
			t_labelsG = policy_theories.selectAll('g.t_labelsG')
				.data(function(d){ return d.nodes(); })
				;
			...
			labelContainer = self.svgG.selectAll('g.labelContainer')
				.data([d3.keys(self.policy_topics)]);
			...
			labelsG
				.style('text-anchor',function(d){
					return self.policy_topics[d].x <self.w/2 ? 'end' : 'start';
				});
			labelsG.exit().remove();
			...
			labels
				.text(function(d){
					return self.policy_topics[d].name;
				});
			labels.exit().remove();
			...
			function topicFocus(_id){
				...
				labelsG
					.classed('highlight',function(d,i){
						return self.policy_topics[d].id === _id;
					})
					.classed('semihighlight',function(d,i){
						return self.policy_topics[d].id === _id && (self.topicFocus && self.policy_topics[d].id !== self.topicFocus);
					})
					.classed('unhighlight',function(d,i){
						return self.policy_topics[d].id !== _id;
					});
			...
			function topicView(_id){
				...
				console.log("update link in the topic view.");
				links.call(update_link);
				hulls.call(update_hull);
				labelsG.call(update_label);
				...
				self.hBox
					...
					/*.html(function(d){
						var quest = self.policy_topics[_id].question,
							descrip = self.policy_topics[_id].description_HTML;
						return '<div class="descrip q">' +quest +'</div>' +'<div class="descrip body">' +descrip +'</div>';
					})*/
					;
				self.hBoxQ
					.html(function(d){
						return self.policy_topics[_id].question;
					});
				self.hBoxBod
					.html(function(d){
						return self.policy_topics[_id].description_HTML;
					});
				...
				self.topicLabel
					.html(function(){
						return '<span>' +self.policy_topics[_id].name +'</span>';
					})
				...
				t_counts
					.classed('hide',function(d){
						return d.policy_topic !== self.topicFocus;
					});

				//set all child theories to hover around the center of the path
				self.t_nodes.forEach(function(d){
					if(d.policy_topic === _id){
						var hull = d3.select('.hull.'+d.policy_topic).node().getBBox();
			...
			function theoryFocus(_d,_time){
				...
				t_nodes
					/*.each(function(d){
						d.fixed = d.id === theory.id && self.topicFocus && d.policy_topic === self.topicFocus;
					})*/
					.classed('hov',function(d){
						return d.id === theory.id && d.policy_topic === theory.topic;
					})
			...
			function getClosestTheory(_x,_y,_topic){
				...
				t_force.nodes().forEach(function(d){
					...
					if(!_topic || (_topic && d.policy_topic === _topic)){
						distances.push(d);
					}
				});
				return distances.sort(function(a,b){ return a.distance -b.distance;})[0];
			...
			// Don't really understand what this function does. -Sosa.
			...
			function update_label(){
				this
					.attr('transform',function(d){
						console.log(d);
						console.log(self.policy_topics[d]);
						var pad = 0,
							x = self.policy_topics[d].fisheye.x <self.w/2 ? self.policy_topics[d].fisheye.x -pad : self.policy_topics[d].fisheye.x +pad,
							y = self.policy_topics[d].fisheye.y,
							p = get_boundedPoint(x,y);
						return 'translate(' +p.x +',' +p.y +')';
					})
					.style('text-anchor',function(d){
						return self.topicFocus ? 'middle' : self.policy_topics[d].x <self.w/2 ? 'end' : 'start';
					});
			}
			...
			function gravity(alpha) {
				return function(d){
					...
					ctr.x = d.focus.x || self.policy_topics[d.policy_topic].x;
					ctr.y = d.focus.y || self.policy_topics[d.policy_topic].y;

					if(!(self.topicFocus && d.policy_topic === self.topicFocus)){
						radius = t_rad;
						buffer = 36 -12 -pad;//self.topicFocus && d.policy_topic === self.topicFocus ? 36 : 36 -12 -pad;
						factor = 1;
						...
						var goal = {},
							hull = d3.select('.hull.' +d.policy_topic).node().getBBox(),
							idx = self.policy_topics[self.topicFocus].policy_theories.indexOf(d.id),
							pad = 18;
						goal.x = ctr.x -(hull.width/6);
						goal.y = (ctr.y -(self.policy_topics[self.topicFocus].policy_theories.length *pad)/2) +pad*idx;
			...
			function collide(alpha) {
				...
				var quadtree = d3.geom.quadtree(self.t_node);
				return function(d){
					if(!(self.topicFocus && d.policy_topic === self.topicFocus)){
						var //r = self.topicFocus ? d.policy_topic === self.topicFocus ? /*c_rad +t_pad*/ 18 : t_rad : t_rad +t_pad,
							r = t_rad +t_pad,
							nx1 = d.x -r,
							nx2 = d.x +r,
							ny1 = d.y -r,
						...
						quadtree.visit(function(quad,x1,y1,x2,y2){
							if(quad.point && (quad.point !==d)){
								var x = d.x -quad.point.x,
									...
									l = Math.sqrt(x*x +y*y),
									r = t_rad +t_pad;
									//r = self.topicFocus ? d.policy_topic === self.topicFocus ? c_rad +t_pad : t_rad : t_rad +t_pad;
								if(l <r){
									l = (l -r)/l*alpha;
									d.x -= x *= l;
									d.y -= y *= l;
			...
			function tick(e){
				console.log("do we get to the ticks function?");
				console.log(links);
				links.call(update_link);
				hulls.call(update_hull);
				labelsG.call(update_label);
				console.log("do we get to the end of ticks?");
			}
			...
			function t_tick(e){
				...
				t_labelsG
					...
					/*.style('text-anchor',function(d){
						return d.x <(d.focus.x || self.policy_topics[d.policy_topic].x) ? 'end' : 'start';
					})*/
					;
				/*t_labels
					.attr('x',function(d){
						return d.x <(d.focus.x || self.policy_topics[d.policy_topic].x) ? -9 : 9;
					});*/
			}
		},
